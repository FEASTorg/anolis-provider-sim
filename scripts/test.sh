...
if [[ "$GENERATE" == true ]]; then
	"$SCRIPT_DIR/generate_proto_python.sh"
fi
...
require_fluxgraph_enabled() {
	...
	if [[ ! -f "$cmake_cache" ]]; then
		echo "[ERROR] FluxGraph suite requires a configured build at: $BUILD_DIR"
		echo "Missing file: $cmake_cache"
		echo "Rebuild with: ./scripts/build.sh --with-fluxgraph"
		exit 1
	fi

	if ! grep -Eq '^ENABLE_FLUXGRAPH:BOOL=ON$' "$cmake_cache"; then
		echo "[ERROR] FluxGraph suite requested but build has ENABLE_FLUXGRAPH=OFF."
		echo "Rebuild with: ./scripts/build.sh --with-fluxgraph"
		exit 1
	fi
}
