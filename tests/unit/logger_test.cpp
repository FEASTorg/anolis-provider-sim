...
TEST_F(LoggerTest, InitFromEnvWarnsOnInvalidValueAndFallsBackToInfo) {
    ...
    const std::string stderr_out = capture_stderr([]() { Logger::init_from_env(); });
    ...
    EXPECT_NE(stderr_out.find("Invalid ANOLIS_PROVIDER_SIM_LOG_LEVEL='not-valid'"), std::string::npos);
}
...
TEST_F(LoggerTest, ThresholdFilteringSuppressesLowerPriorityMessages) {
    ...
    const std::string stderr_out = capture_stderr([]() {
        Logger::log(LogLevel::Info, "UnitTest", __FILE__, __LINE__, "this should not be emitted");
        Logger::log(LogLevel::Error, "UnitTest", __FILE__, __LINE__, "this should be emitted");
    });
