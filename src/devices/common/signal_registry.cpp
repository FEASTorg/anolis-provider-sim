...
void SignalRegistry::set_device_reader(std::function<std::optional<double>(const std::string &)> reader) {
    std::lock_guard<std::mutex> lock(registry_mutex_);
...
std::optional<double> SignalRegistry::get_cached_value(const std::string &path) const {
    std::lock_guard<std::mutex> lock(registry_mutex_);
    auto it = signal_cache_.find(path);
    if (it != signal_cache_.end()) {
        return it->second;
