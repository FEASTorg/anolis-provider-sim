...
static std::optional<double> parse_double(const std::map<std::string, YAML::Node> &config, const std::string &key) {
...
static std::optional<std::pair<double, double>> parse_range(const std::map<std::string, YAML::Node> &config,
...
bool DeviceFactory::initialize_device(const DeviceSpec &spec) {
    if (spec.type == "tempctl") {
        ...
        PSIM_LOG_INFO("DeviceFactory", "Initialized device '" << spec.id << "' (type: relayio)");
        return true;
        ...
        PSIM_LOG_INFO("DeviceFactory", "Initialized device '" << spec.id << "' (type: analogsensor)");
        return true;
    } else {
