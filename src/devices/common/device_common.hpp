...
static inline bool value_is_type(const Value &v, ValueType t) { return v.type() == t; }

static inline bool get_arg_bool(const std::map<std::string, Value> &args, const char *key, bool &out) {
    auto it = args.find(key);
    if (it == args.end()) return false;
    if (!value_is_type(it->second, ValueType::VALUE_TYPE_BOOL)) return false;
...
static inline bool get_arg_int64(const std::map<std::string, Value> &args, const char *key, int64_t &out) {
    auto it = args.find(key);
    if (it == args.end()) return false;
    if (!value_is_type(it->second, ValueType::VALUE_TYPE_INT64)) return false;
...
static inline bool get_arg_double(const std::map<std::string, Value> &args, const char *key, double &out) {
    auto it = args.find(key);
    if (it == args.end()) return false;
    if (!value_is_type(it->second, ValueType::VALUE_TYPE_DOUBLE)) return false;
...
static inline bool get_arg_string(const std::map<std::string, Value> &args, const char *key, std::string &out) {
    auto it = args.find(key);
    if (it == args.end()) return false;
    if (!value_is_type(it->second, ValueType::VALUE_TYPE_STRING)) return false;
...
static inline ArgSpec make_arg_spec(const std::string &name, ValueType type, bool required,
...
static inline FunctionPolicy make_function_policy(FunctionPolicy::Category category, bool requires_lease = false,
...
static inline CallResult out_of_range(const std::string &m) { return {Status::CODE_OUT_OF_RANGE, m}; }
