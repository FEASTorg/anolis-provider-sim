...
namespace sim_devices
{
    namespace tempctl
    {
        ...
        void update_physics(double dt)
        {
            ...
            if (s.mode == "closed")
            {
                // In closed-loop, pretend relays/power are modulated to move toward setpoint.
                // We still keep relay states meaningful by modeling a simple "effective heating".
                target = s.setpoint_c;
            }
            else
            {
        ...
        CapabilitySet get_capabilities()
        {
            ...
            // Functions
            {
                ...
                *f.add_args() = make_arg("mode", ValueType::VALUE_TYPE_STRING, true, "open or closed");
            ...
            }
            {
                ...
                *f.mutable_policy() = make_policy(FunctionPolicy::CATEGORY_CONFIG);
                auto a = make_arg("value", ValueType::VALUE_TYPE_DOUBLE, true, "Temperature setpoint", "C");
            ...
            }
            {
                ...
                *f.mutable_policy() = make_policy(FunctionPolicy::CATEGORY_ACTUATE);
                ...
                auto idx = make_arg("relay_index", ValueType::VALUE_TYPE_INT64, true, "1 or 2");
                idx.set_min_int64(1);
                idx.set_max_int64(2);
                *f.add_args() = idx;

                *f.add_args() = make_arg("state", ValueType::VALUE_TYPE_BOOL, true, "true=on false=off");
                *caps.add_functions() = f;
            }
