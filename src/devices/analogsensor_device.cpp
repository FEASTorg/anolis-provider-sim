        ...
        CapabilitySet get_capabilities()
        {
            ...
            // Functions
            {
                ...
                *f.mutable_policy() = make_policy(FunctionPolicy::CATEGORY_CONFIG);
                auto a = make_arg("channel", ValueType::VALUE_TYPE_INT64, true, "Channel index (1 or 2)");
                a.set_min_int64(1);
                a.set_max_int64(2);
                *f.add_args() = a;
            ...
            }
            {
                ...
                *f.mutable_policy() = make_policy(FunctionPolicy::CATEGORY_CONFIG);
                *f.add_args() = make_arg("enabled", ValueType::VALUE_TYPE_BOOL, true, "Enable/disable noise");
                *caps.add_functions() = f;
            }
        ...
        std::vector<SignalValue> read_signals(const std::vector<std::string> &signal_ids)
        {
            ...
            for (const auto &id : ids)
            {
                if (id == kSigVoltageCh1)
                    out.push_back(make_signal_value(id, make_double(get_noisy_reading(s, s.voltage_ch1_base))));
                else if (id == kSigVoltageCh2)
                    out.push_back(make_signal_value(id, make_double(get_noisy_reading(s, s.voltage_ch2_base))));
