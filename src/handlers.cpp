...
namespace handlers
{
    ...
    void handle_hello(const HelloRequest &req, anolis::deviceprovider::v0::Response &resp)
    {
    ...
    void handle_list_devices(const ListDevicesRequest &req, anolis::deviceprovider::v0::Response &resp)
    {
        ...
        // v0 sim: we ignore include_health for now (device_health omitted intentionally).
        set_status_ok(resp);
    }

    void handle_describe_device(const DescribeDeviceRequest &req, anolis::deviceprovider::v0::Response &resp)
    {
    ...
    void handle_read_signals(const ReadSignalsRequest &req, anolis::deviceprovider::v0::Response &resp)
    {
        if (req.device_id().empty())
        {
            set_status(resp, Status::CODE_INVALID_ARGUMENT, "device_id is required");
        ...
        {
            // v0 policy: if explicit ids were requested and none are returned, treat as NOT_FOUND
            set_status(resp, Status::CODE_NOT_FOUND, "no requested signals found for device");
            return;
        }
    ...
    void handle_call(const CallRequest &req, anolis::deviceprovider::v0::Response &resp)
    {
        if (req.device_id().empty())
        {
            set_status(resp, Status::CODE_INVALID_ARGUMENT, "device_id is required");
        ...
        {
            set_status(resp, Status::CODE_UNIMPLEMENTED, "function_name lookup not implemented in sim provider v0");
            return;
        }
        ...
        const auto result = sim_devices::call_function(req.device_id(), req.function_id(), args);
    ...
    void handle_get_health(const GetHealthRequest & /*req*/, anolis::deviceprovider::v0::Response &resp)
    {
        auto *out = resp.mutable_get_health();
        *out->mutable_provider() = sim_health::make_provider_health_ok();

        // v0 sim: device health is optional; include basic OK states for known devices
        const auto devices = sim_devices::list_devices(false);
        for (const auto &d : devices)
        {
