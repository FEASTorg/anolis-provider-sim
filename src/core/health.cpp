...
ProviderHealth make_provider_health(const DeviceInitializationReport &report) {
    ...
    const std::size_t inferred_failed =
        report.configured_device_count > initialized ? (report.configured_device_count - initialized) : 0U;
    const std::size_t failed = explicit_failed > inferred_failed ? explicit_failed : inferred_failed;
    ...
    (*health.mutable_metrics())["startup_policy"] = startup_policy_name(report.startup_policy);
    (*health.mutable_metrics())["startup_configured_devices"] = std::to_string(report.configured_device_count);
    (*health.mutable_metrics())["startup_initialized_devices"] = std::to_string(initialized);
    (*health.mutable_metrics())["startup_failed_devices"] = std::to_string(failed);
...
std::vector<DeviceHealth> make_get_health_devices(const DeviceInitializationReport &report) {
    ...
    std::set<std::string> success_ids(report.successful_device_ids.begin(), report.successful_device_ids.end());

    const auto listed_devices = sim_devices::list_devices(false);
    out.reserve(listed_devices.size() + report.failed_devices.size() + report.configured_device_ids.size());
