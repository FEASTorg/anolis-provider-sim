...
public:
    ...
    void register_provider(const std::string &provider_name, const std::vector<std::string> &device_ids) override;

    bool update_signals(const std::map<std::string, double> &actuators, const std::string &unit,
                        std::chrono::milliseconds timeout) override;

    std::map<std::string, double> read_signals(const std::vector<std::string> &signal_paths) override;
