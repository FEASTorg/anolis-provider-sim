...
void set_rpc_deadline(grpc::ClientContext &ctx, std::chrono::milliseconds timeout) {
    if (timeout.count() <= 0) {
...
FluxGraphClient::FluxGraphClient(const std::string &server_address) {
    ...
    if (!channel_->WaitForConnected(deadline)) {
        throw std::runtime_error("Failed to connect to FluxGraph server at " + server_address);
    }
}
...
void FluxGraphClient::load_config_content(const std::string &yaml_content) {
    ...
    if (!rpc_ok) {
        throw std::runtime_error(
            "LoadConfig RPC failed: code=" + std::to_string(static_cast<int>(status.error_code())) +
...
void FluxGraphClient::register_provider(const std::string &provider_id, const std::vector<std::string> &device_ids) {
    check_connection();
    ...
    if (!rpc_ok) {
        throw std::runtime_error(
            "RegisterProvider RPC failed: code=" + std::to_string(static_cast<int>(status.error_code())) +
            " message=" + status.error_message());
    }
    if (!res.success()) {
...
bool FluxGraphClient::update_signals(const std::map<std::string, double> &signals, const std::string &default_unit,
    ...
    const std::chrono::milliseconds rpc_timeout = (timeout.count() > 0) ? timeout : kDefaultRpcTimeout;
    ...
    for (int attempt = 0; attempt < kMaxRpcAttempts; ++attempt) {
        ...
        if (status.error_code() == grpc::StatusCode::UNAUTHENTICATED && !retried_after_unauth) {
            registered_ = false;
            session_id_.clear();
    ...
    throw std::runtime_error("UpdateSignals RPC failed: code=" + std::to_string(static_cast<int>(status.error_code())) +
                             " message=" + status.error_message());
}

std::optional<FluxGraphClient::SignalInfo> FluxGraphClient::read_signal(const std::string &path) {
    ...
    for (int attempt = 0; attempt < kMaxRpcAttempts; ++attempt) {
        ...
        if (status.error_code() == grpc::StatusCode::UNAUTHENTICATED && !retried_after_unauth && registered_) {
            retried_after_unauth = true;
            if (try_reregister_session()) {
                continue;
            }
...
void FluxGraphClient::reset() {
    ...
    if (!rpc_ok) {
        throw std::runtime_error("Reset RPC failed: code=" + std::to_string(static_cast<int>(status.error_code())) +
                                 " message=" + status.error_message());
    }
    if (!res.success()) {
