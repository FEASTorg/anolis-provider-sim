...
void FluxGraphAdapter::load_config(const std::string &config_path) {
    ...
    std::string fluxgraph_yaml((std::istreambuf_iterator<char>(file)), std::istreambuf_iterator<char>());
...
void FluxGraphAdapter::register_provider(const std::string &provider_name, const std::vector<std::string> &device_ids) {
...
bool FluxGraphAdapter::update_signals(const std::map<std::string, double> &actuators, const std::string &unit,
                                      std::chrono::milliseconds timeout) {
    return client_->update_signals(actuators, unit, timeout);
}

std::map<std::string, double> FluxGraphAdapter::read_signals(const std::vector<std::string> &signal_paths) {
...
std::vector<std::string> FluxGraphAdapter::list_signals() { return output_paths_; }
