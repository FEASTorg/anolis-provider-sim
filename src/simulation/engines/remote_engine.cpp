...
RemoteEngine::RemoteEngine(std::unique_ptr<sim_adapters::ProtocolAdapter> adapter, double tick_rate_hz)
    : adapter_(std::move(adapter)), tick_rate_hz_(tick_rate_hz) {}

void RemoteEngine::initialize(const std::string &config_path) { adapter_->load_config(config_path); }
...
TickResult RemoteEngine::tick(const std::map<std::string, double> &actuators) {
    try {
        ...
        const bool success = adapter_->update_signals(actuators, "dimensionless", timeout);
        if (!success) {
...
std::vector<std::string> RemoteEngine::list_signals() { return adapter_->list_signals(); }
